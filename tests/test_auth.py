...
def sqlite_session_factory(monkeypatch):
    ...
    from src.schema import db_models
...
def test_require_admin_rejects_non_admin(sqlite_session_factory):
    ...
    async def _do():
        async with sqlite_session_factory() as session:
            user = User(email="a@b.com", hashed_password="x", full_name="A", role="user")
            session.add(user)
...
def test_require_admin_allows_admin(sqlite_session_factory):
    ...
    async def _do():
        async with sqlite_session_factory() as session:
            user = User(email="admin@b.com", hashed_password="x", full_name="A", role="admin")
            session.add(user)
            await session.commit()
            await session.refresh(user)
