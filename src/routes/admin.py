...
def add_item(item: ItemRequest, user_id:str=Depends(get_current_user)):
    if user_id!="admin":
...
def remove_item(item:ItemRequest, user_id:str=Depends(get_current_user)):
    if user_id!="admin":
        raise HTTPException(403, "Admin privileges required")
...
def show_list(user_id:str=Depends(get_current_user)):
    if user_id!="admin":
        raise HTTPException(403, "Admin privileges required")
